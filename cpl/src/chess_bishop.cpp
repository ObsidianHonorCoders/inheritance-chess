...
/// @details   Implementation of ChessBishop class with complete bishop movement logic.
...
namespace Chess
{
  ...
  const int ChessBishop::bishop_directions_[4][2] = {
  ...
  ChessBishop::ChessBishop(Color color, const Position& pos)
      : color_(color), position_(pos)
  {
      if (!pos.is_valid())
      {
          throw std::invalid_argument("Invalid position for ChessBishop");
      }
  }

  /// @brief Get the character representation of the piece.
  /// @return 'B' for white bishops, 'b' for black bishops.
  char ChessBishop::get_representation() const
  {
  ...
  void ChessBishop::available_moves(PositionList& moves,
                                 ...
                                 const Grid& grid) const
  {
      ...
      for (int i = 0; i < 4; i++)
      {
          int file_offset = bishop_directions_[i][0];
          int rank_offset = bishop_directions_[i][1];
          
          // Move step by step in the diagonal direction
  ...
  bool ChessBishop::can_move_to(const Position& target_pos,
                               const PositionList& other_positions,
                               const ColorList& other_colors) const
  {
      auto it = std::find(other_positions.begin(), other_positions.end(), target_pos);
      
      // If square is empty, bishop can move there
      if (it == other_positions.end())
      {
          return true;
      }
      
      // If square is occupied, bishop can only capture opponent pieces
  ...
  bool ChessBishop::is_path_clear(const Position& target_pos,
                                 const PositionList& other_positions) const
  {
      ...
      for (int distance = 1; distance < abs(file_diff); distance++)
      {
