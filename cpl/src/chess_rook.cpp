/// @file      chess_rook.cpp
/// @namespace Chess
/// @brief     Chess Pieces Logic (CPL) - Rook Implementation.
/// @author    Calileus
/// @date      2026-01-22
/// @copyright 2026 Obsidian Honor Coders. Licensed under Apache 2.0.
/// @see       https://github.com/ObsidianHonorCoders/inheritance-chess
/// @details   Implementation of ChessRook class with complete rook movement logic.
...
namespace Chess
{
  ...
  /// @brief Rook move directions (4 orthogonal directions).
  /// @details Array of 4 orthogonal directions: (file_offset, rank_offset)
  const int ChessRook::rook_directions_[4][2] = {
  ...
  /// @brief Construct a ChessRook at the specified position and color.
  /// @param color The color of the rook (WHITE or BLACK).
  /// @param pos The initial position of the rook.
  ChessRook::ChessRook(Color color, const Position& pos)
      : color_(color), position_(pos)
  {
      if (!pos.is_valid())
      {
          throw std::invalid_argument("Invalid position for ChessRook");
      }
  }

  /// @brief Get the character representation of the piece.
  /// @return 'R' for white rooks, 'r' for black rooks.
  char ChessRook::get_representation() const
  {
  ...
  /// @details Rooks move horizontally or vertically but are blocked by pieces.
  void ChessRook::available_moves(PositionList& moves,
                                 ...
                                 const Grid& grid) const
  {
      ...
      for (int i = 0; i < 4; i++)
      {
          int file_offset = rook_directions_[i][0];
          int rank_offset = rook_directions_[i][1];
          
          // Move step by step in the orthogonal direction
  ...
  /// @brief Check if a rook can move to the target position.
  ...
  bool ChessRook::can_move_to(const Position& target_pos,
                             const PositionList& other_positions,
                             const ColorList& other_colors) const
  {
      auto it = std::find(other_positions.begin(), other_positions.end(), target_pos);
      
      // If square is empty, rook can move there
      if (it == other_positions.end())
      {
          return true;
      }
      
      // If square is occupied, rook can only capture opponent pieces
  ...
  bool ChessRook::is_path_clear(const Position& target_pos,
                               const PositionList& other_positions) const
  {
      ...
      // Verify it's actually an orthogonal move (not diagonal)
