...
namespace Chess
{
  ...
  /// @class ChessRook
  /// @brief Represents a Rook chess piece in the CPL module.
  /// @details Inherits from ChessPiece and implements rook-specific movement rules.
  ...
  public:
      ...
      ChessRook(Color color, const Position& pos);

      /// @brief Virtual destructor for proper cleanup.
      virtual ~ChessRook() override = default;

      /// @brief Get the type of the piece.
      /// @return Always returns PieceType::ROOK.
  ...
  private:
      Color color_;    ///< Color of the rook (WHITE or BLACK)
      Position position_; ///< Current position of the rook

      /// @brief Rook move directions (4 orthogonal directions).
      /// @details Array of 4 orthogonal directions: (file_offset, rank_offset)
      static const int rook_directions_[4][2];
